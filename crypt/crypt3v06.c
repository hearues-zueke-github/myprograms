...
void encryption(uchar matrix[MS][MS][MS][MS], uchar *crypt, int crypt_length);
void decryption(uchar matrix[MS][MS][MS][MS], uchar *crypt, int crypt_length);
...
void printMatrix(uchar matrix[MS][MS][MS][MS], int length)
{
	...
	for (loop = 0; loop < length; loop++)
	{
		if ((loop != 0) && (loop % 16 != 0))
		{
			if (loop % 16 != 15)
			{
				printf(" %02x", matrix[(loop / (MS*MS*MS)) % MS][(loop / (MS*MS)) % MS][(loop / MS) % MS][loop % MS]);
			}
			else
			{
		...
		{
			printf("0x%02x   %02x", loop / 16, matrix[(loop / (MS*MS*MS)) % MS][(loop / (MS*MS)) % MS][(loop / MS) % MS][loop % MS]);
		}
	}
	printf("\n");
}
	...
	while (loop_file < argc) {
		...
		if (file_in != NULL) {
			...
			while (file_in_position > 0) {
				...
				// encrypt algorithm
				encryption(matrix, crypt, crypt_size);
				// write out
				if (pos > 0) {
					int loop = 0;
		...
		if (file_in != NULL) {
			...
			if (file_size_number <= file_in_position) {
				while (file_in_position > 0) {
					...
					// decrypt algorithm
					if (file_in_position < 256*2)
					{
						printMatrix(matrix, MS*MS*MS*MS);
					}
					decryption(matrix, crypt, crypt_size);
					if (file_in_position < 256*2)
					{
						printMatrix(matrix, MS*MS*MS*MS);
					}
